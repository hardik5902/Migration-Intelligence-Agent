...
from tools.duckdb_tools import active_cache_backend, clear_cache, db_path, get_cache_overview
    ...
    if not _auth_ready():
        def _auth_err():
            yield _sse("error", {"message": (
                ...
                "Configure Vertex AI env vars or set GOOGLE_API_KEY / GOOGLE_APPLICATION_CREDENTIALS."
            )})
...
def cache_viewer() -> str:
    """Browser-based cache inspector for the active backend."""
    tables_info, samples, error = get_cache_overview()

    return render_template(
        "cache.html",
        tables=tables_info,
        samples=samples,
        db_path=f"{active_cache_backend()}: {db_path()}",
        error=error,
    )
